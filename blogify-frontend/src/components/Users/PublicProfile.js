...
import { blockUserAction, followUserAction, unBlockUserAction, unFollowUserAction, userPrivateProfileAction, userPublicProfileAction } from "../../redux/slices/users/usersSlice";
...
export default function PublicProfile() {
  ...
  //dispatch public profile action
  ...
  // check if the public profile user is blocked
  const hasBlocked = profile?.user?.blockedUsers?.includes(id);
  console.log(hasBlocked);

  //check if the public profile user is followed
  const hasFollowed = profile?.user?.following?.includes(id);
  console.log(hasFollowed);

  //private profile action
  useEffect(() => {
    dispatch(userPrivateProfileAction());
  }, [id, dispatch,hasBlocked,hasFollowed]);
