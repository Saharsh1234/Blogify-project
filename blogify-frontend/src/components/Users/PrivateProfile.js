...
import { sendAccVerificationEmailAction, userPrivateProfileAction } from "../../redux/slices/users/usersSlice";
...
export default function PrivateUserProfile() {
  ...
  //dispatch private profile action
  useEffect(() => {
    dispatch(userPrivateProfileAction());
  }, [dispatch]);
  const { profile, user, loading, error, userAuth, isEmailSent } = useSelector((state) => state?.users)
