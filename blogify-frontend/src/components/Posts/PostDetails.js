...
import { getPostAction,postViewsCounttAction } from "../../redux/slices/posts/postsSlice";
...
const PostDetails = () => {
  ...
  const creater=post?.post?.author?._id;
  console.log(creater);
  //get the login user
  const loginUser=userAuth?.userInfo?.id;
  console.log(loginUser);

  const isCreater=JSON.stringify(creater)===JSON.stringify(loginUser)
          ...
          >
            <div className="container px-4 mx-auto">
              <div className="mx-auto mb-12 text-center md:max-w-2xl">
                ...
                <Link
                to={`/public-profile/${post?.post?.author?._id}`} 
